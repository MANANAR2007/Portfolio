...
function previewDocument(project: Project) {
  const cards = project.previewCards
    .map(
      (card) => `
        ...
        <style>
          * { box-sizing: border-box; }
          body {
            ...
            background:
              radial-gradient(circle at 20% 15%, ${project.accentGlow}, transparent 34%),
              radial-gradient(circle at 84% 72%, rgba(255,255,255,0.09), transparent 28%),
          ...
          .pulse {
            height: 10px;
            border-radius: 999px;
            background: linear-gradient(90deg, ${project.accent}, rgba(255,255,255,0.3));
            box-shadow: 0 0 30px ${project.accentGlow};
            transform-origin: left;
            animation: pulse 2.8s ease-in-out infinite;
          }
          @keyframes pulse {
            0%, 100% { transform: scaleX(0.42); opacity: 0.62; }
      ...
      <body>
        <main class="shell">
          ...
          <section class="hero">
            <div class="main-card">
              <div>
                <h1>${project.previewTitle}</h1>
                <p>${project.previewSubtitle}</p>
              </div>
